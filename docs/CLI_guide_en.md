...
| `--distance <rgb|hsv>` | Color distance mode for palette selection. Default: `hsv`. |
| `--weight-h`, `--weight-s`, `--weight-b` | Weights (0–1) for hue, saturation, and brightness when `hsv` distance is selected. |
